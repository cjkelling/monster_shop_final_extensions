...
describe 'Admin user can see all merchants' do
  before :each do
    ...
    @dog_shop = Merchant.create(name: "Brian's Dog Shop", address: '125 Doggo St.', city: 'Denver', state: 'CO', zip: 80_210, enabled?: true)
    ...
    @order_1 = @regular_user.orders.create(address_id: @regular_user_address.id, status: 0)
    @order_2 = @regular_user.orders.create(address_id: @regular_user_address.id, status: 0)
  ...
  it 'Admin can see merchant dashboard' do
    ...
    expect(page).to have_content(@order_1.created_at.strftime('%d %b %y').to_s)
    ...
    expect(page).to have_content(@bike_shop.name.to_s)
    expect(page).to have_content(@bike_shop.address.to_s)
    expect(page).to have_content(@bike_shop.city.to_s)
    expect(page).to have_content(@bike_shop.state.to_s)
    expect(page).to have_content(@bike_shop.zip.to_s)
  end
